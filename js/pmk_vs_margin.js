var dataset_pmk = [];
		d3.csv("csv/pmk_vs_margin.csv", function(data) {

  		data.forEach(function(d) {
		dataset_pmk.push([+d['CONUM'], +d['LOG(MAR)']*100,d["WINPARTY"],d["TYPE1"],+d["MARGIN"],+d["PMK"],d["CONAME"],+d["RECTPOS"], +d['LOG(PMK)']*100, +d["INDEX"], +d["FLAG"]])
    		dataset_pmk.push([+d['CONUM'], +d['LOG(PMK)']*100,d["WINPARTY"],d["TYPE2"],+d["MARGIN"],+d["PMK"],d["CONAME"],+d["RECTPOS"], +d['LOG(MAR)']*100, +d["INDEX"]+1, +d["FLAG"]+1])
		});
		...
		var rect = svg.selectAll("rect")
					  ...
					  .on("mouseover", function(d) {	
											...
											div.html("<font size=3>" + "<b>" + d[6] + "</b>" + "</font>" + "<br/>" + d[2] + "<br/>" + "<br/>" + "Winning Margin " + "&nbsp;" + d[4] + "<br/>" + "PMK " + "&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; " + d[5]).style("left", (mouse[0] + 25) + "px").style("top", (mouse[1] - 28) + "px");	
											if(d[1]<d[8]) {
		...
		var axisavg_pmk = [ [1,100], [235,100], [1,200], [235,200], [1,300], [235,300], [1,400], [235,400], [1,500], [235,500], [1, 100], [1, 500], [1,358], [235, 358] ];
		 
		var aia = [{val:"AIADMK+",xpos:1200,ypos:27,col:"black",size:"15"},{val:"DMK+",xpos:1200,ypos:48,col:"black",size:"15"},{val:"10",xpos:25,ypos:510, col:"black",size:"15"},{val:"100",xpos:18,ypos:410,col:"black",size:"15"},{val:"1000",xpos:13,ypos:310,col:"black",size:"15"},{val:"10000",xpos:7,ypos:210,col:"black",size:"15"},{val:"100000",xpos:1,ypos:110,col:"black",size:"15"},{val:"10",xpos:1220,ypos:510,col:"black",size:"15"},{val:"100",xpos:1220,ypos:410,col:"black",size:"15"},{val:"1000",xpos:1220,ypos:310,col:"black",size:"15"},{val:"10000",xpos:1220,ypos:210,col:"black",size:"15"},{val:"100000",xpos:1220,ypos:110,col:"black",size:"15"},{val:"Avg. PMK",xpos:1220,ypos:225,col:"blue",size:"13"}, {val:"VOTES",xpos:8,ypos:260,col:"gray",size:"15"}, {val:"CONSTITUENCIES",xpos:620,ypos:524,col:"gray",size:"15"},{val:"CONSTITUENCY-WISE EFFECT OF THE PMK", xpos:54,ypos:27,col:"black",size:"18"}, {val:"The chart shows the number of votes that the PMK polled in each constituency in descending order of magnitude. The points above the curve indicate the constituencies where the victory", xpos:54,ypos:50,col:"gray",size:"14"}, {val:"margins are more than the PMK votes polled, while the points below the curve, vice-versa. The length of the lines indicate the quantum of the difference. Votes along the Y-axis are shown", xpos:54,ypos:66,col:"gray",size:"14"}, {val:" on a logarithmic scale to allow for comparison.", xpos:54,ypos:83,col:"gray",size:"14"}, {val:"Hover mouse cursor over the chart to explore constituency details.", xpos:315,ypos:83,col:"#d92b2b",size:"14"}];
		...
		for (var i=1; i<dataset_pmk.length; i=i+1) {
		
			var v1 = dataset_pmk[i][0];
			var v2 = dataset_pmk[i][1];
		...
		for (var i=0; i<dataset_pmk.length; i=i+2) {
		
			var v1 = dataset_pmk[i][0];
			var v2 = dataset_pmk[i][1];
			var v3 = dataset_pmk[i+1][0];
			var v4 = dataset_pmk[i+1][1];
			var v5 = dataset_pmk[i][2];
			var v6 = dataset_pmk[i][3];
			var v7 = dataset_pmk[i][4];
			var v8 = dataset_pmk[i+1][3];
			var v9 = dataset_pmk[i+1][5];
			var v10 = dataset_pmk[i][6];
			
			drawLine([{"x": v1, "y": v2, "p": v5, "t": v6, "v": v7, "c": v10}, {"x": v3, "y": v4, "p": v5, "t": v8, "v": v9, "c": v10}])
		};
		
		for (var i=0; i<axisavg_pmk.length; i=i+2) {
		
			var v1 = axisavg_pmk[i][0];
			var v2 = axisavg_pmk[i][1];
			var v3 = axisavg_pmk[i+1][0];
			var v4 = axisavg_pmk[i+1][1];
